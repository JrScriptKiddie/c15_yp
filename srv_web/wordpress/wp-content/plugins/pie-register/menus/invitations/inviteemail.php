    ...
    <div class="cols-3">          
      <select required id="pie_email_linkpage" name="pie_email_linkpage" >
        ...
        foreach( $pages as $page ) : $page->post_content; ?>
          <option value="<?php echo esc_attr($page->ID); ?>" <?php echo ($piereg['pie_email_linkpage'] == $page->ID) ? 'selected' : ''; ?>>
...
<li class="clearfix">
  <div class="fields extra-margin">
    <div class="cols-3">
      ...
      <span class="quotation import-email-invites"><?php echo sprintf( __( 'You may want to see', 'pie-register').' <a target="_blank" download href="%s"> '.__('this example of the CSV file.', 'pie-register').'</a>. Use a separate line for each email address.' , plugin_dir_url(__FILE__).'../examples/example-import-email-addresses-test.csv'); ?>
      </span>
    </div>
  </div>
</li>
  ...
  <ul class="bg-white clearfix invite-form custom-template">
      ...
      <li class="clearfix">
        <div class="fields">
         ...
         <div class="cols-3">          
          <textarea class="invite-email-text" name="pie_email_content" rows="10" cols="50"><?php echo (isset($piereg['pie_email_content'])?$piereg['pie_email_content']:''); ?></textarea>
