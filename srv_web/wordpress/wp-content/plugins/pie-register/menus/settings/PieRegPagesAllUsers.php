  ...
  if( function_exists( 'wp_nonce_field' )) wp_nonce_field( 'piereg_wp_settings_allusers','piereg_settings_allusers'); ?>
       <div class="fields" style="float:none;">
            <div class="radio_fields">
                 <input <?php echo $_disable; ?> type="checkbox" 
                    name="login_after_register" id="login_after_register_yes" value="1" <?php echo ($piereg['login_after_register']=="1")?'checked="checked"':''?> />
            </div>
            <label class="label_mar_top" for="login_after_register_yes"><?php _e("Auto login users after registration.",'pie-register'); echo $_available_in_pro; ?></label> <br /><br />
    ...
    <div class="fields">
      ...
      <select id="after_login" name="after_login" >
        ...
        <?php foreach( $pages as $page ) : $page->post_content; ?>
          <option class="level-0" value="<?php echo $page->ID; ?>" <?php if($page->ID == $piereg['after_login']){ echo "selected"; } ?>>
            <?php echo $page->post_title; ?>
          </option>
        <?php endforeach; ?>
        <option value="" disabled> ---- Posts ---- </option>
        <?php foreach( $posts as $post ) : setup_postdata($post); ?>
          <option class="level-0" value="<?php echo $post->ID; ?>" <?php if($post->ID == $piereg['after_login']){ echo "selected"; } ?>>
            <?php the_title(); ?>
          </option>
        <?php endforeach; ?>
    ...
    <div class="fields <?php echo ($piereg['after_login'] == "url") ? "": "hide"; ?>">
      ...
      <input type="text" name="alternate_login_url" id="alternate_login_url" value="<?php echo (isset($piereg['alternate_login_url']) ? $piereg['alternate_login_url'] : "" ); ?>" class="input_fields" />
    </div>
    <div class="fields">
      ...
      <select id="alternate_logout" name="alternate_logout" >
        ...
        <option value="" disabled> ---- Pages ---- </option>
        <?php foreach( $pages as $page ) : $page->post_content; ?>
          <option class="level-0" value="<?php echo $page->ID; ?>" <?php if($page->ID == $piereg['alternate_logout']){ echo "selected"; } ?>>
            <?php echo $page->post_title; ?>
          </option>
        <?php endforeach; ?>
        <option value="" disabled> ---- Posts ---- </option>
        <?php foreach( $posts as $post ) : setup_postdata($post); ?>
          <option class="level-0" value="<?php echo $post->ID; ?>" <?php if($post->ID == $piereg['alternate_logout']){ echo "selected"; } ?>>
            <?php the_title(); ?>
          </option>
        <?php endforeach; ?>
    ...
    <div class="fields <?php echo ($piereg['alternate_logout'] == "url") ? "": "hide"; ?>">
      ...
      <input type="text" name="alternate_logout_url" id="alternate_logout_url" value="<?php echo (isset($piereg['alternate_logout_url']) ? $piereg['alternate_logout_url'] : "" ); ?>" class="input_fields" />
    </div>    
