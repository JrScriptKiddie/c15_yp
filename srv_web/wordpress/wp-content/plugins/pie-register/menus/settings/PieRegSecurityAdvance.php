...
if( !isset($_GET['act']) || !isset($_GET['pie_id']) || !isset($_GET['option']) )
{
    ...
    <?php if( function_exists( 'wp_nonce_field' )) wp_nonce_field( 'piereg_wp_settings_security_advanced','piereg_settings_security_advanced'); ?>
        <div class="fields">
            ...
            <span class="quotation" style="margin-left:0px;"><?php _e("Security enhancement for forms (timed submission)",'pie-register') ?><?php echo $_available_in_pro; ?></span>
            </div>
        </div>
        <div class="fields">
        	<div class="piereg_box_style_1 limit-submission">
             <?php _e("Form Submission Limit for a Device ",'pie-register') ?>
             <input <?php echo $_disable; ?> type="text" name="reg_form_submission_limit" 
             		style="width:auto;"
                    id="reg_form_submission_limit" 
                    value="<?php echo ( (isset($piereg['reg_form_submission_limit']) && !empty($piereg['reg_form_submission_limit'])) ? intval($piereg['reg_form_submission_limit']) : 0 ); ?>" 
                    class="input_fields submissionfield" 
                    /><br>
            <span class="quotation" style="margin-left:0px;"><?php _e("Limits how many times a form can be submitted from a device within a day. Helpful to prevent spams. Set it to zero(0) to disable this feature.",'pie-register') ?><?php echo $_available_in_pro; ?></span>
            </div>
        </div>
        <div class="fields">
