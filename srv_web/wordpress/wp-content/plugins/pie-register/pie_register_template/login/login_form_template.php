	...
	class Login_form_template
	{
		...
		function add_username(){
			...
			if(isset($this->pr_option['login_username_label']) && !empty($this->pr_option['login_username_label'])){
					$form_data .= '<label for="user_login">'.((isset($this->pr_option['login_username_label']) && !empty($this->pr_option['login_username_label']))? __($this->pr_option['login_username_label'],"pie-register") : __("Username","pie-register")) .'</label>';
			}
			$user_name_val = ((isset($_POST['log']) && !empty($_POST['log']))? sanitize_user($_POST['log']):"");
			$form_data .= '<input placeholder="'.((isset($this->pr_option['login_username_placeholder']) && !empty($this->pr_option['login_username_placeholder']))? __($this->pr_option['login_username_placeholder'],"pie-register") : "").'" type="text" value="'.$user_name_val.'" class="input input_fields piereg_validate[required]" id="user_login" name="log">';
			$form_data .= '</div>';
			return $form_data;
		}
		function add_password(){
			...
			if(isset($this->pr_option['login_password_label']) && !empty($this->pr_option['login_password_label'])){
				$form_data .= '<label for="user_pass">'.((isset($this->pr_option['login_password_label']) && !empty($this->pr_option['login_password_label']))? __($this->pr_option['login_password_label'],"pie-register") : __("Password","pie-register")).'</label>';
			}
			$form_data .= '<input placeholder="'.((isset($this->pr_option['login_password_placeholder']) && !empty($this->pr_option['login_password_placeholder']))? __($this->pr_option['login_password_placeholder'],"pie-register") : "").'" type="password" value="" class="input input_fields piereg_validate[required]" id="user_pass" name="pwd">';
		...
		function add_register_lostpassword($pagenow){
			$form_data = '<p id="nav">';
				...
				if($classPieRegister->is_anyone_can_register()){
					$form_data .= '<a href="'.pie_registration_url().'">'.__("Register","pie-register").'</a>';
					$form_data .= '<a style="cursor:default;text-decoration:none;" href="javascript:;"> | </a>';
				}
				$form_data .= '<a title="'.__("Reset your password","pie-register").'" href="'.pie_lostpassword_url().'">'.__("Lost your password?","pie-register").'</a>';
			$form_data .= '</p>';
			if(isset($pagenow) && $pagenow == 'wp-login.php' ){
				$form_data .= '<p id="backtoblog">';
					$form_data .= '<a title="'.__("Are you lost?","pie-register").'" href="'.get_bloginfo("url").'">&larr;'.__(" Back to ".get_bloginfo("name"),"pie-register").'</a>';
				$form_data .= '</p>';
			}
