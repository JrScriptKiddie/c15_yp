...
global $wp_session;
            ...
            if ( 'loggedout' == $_GET['action'] )
                $message = __("You are now logged out.","pie-register");
            elseif ( 'recovered' == $_GET['action'] )
                $message = __("Check your e-mail for the confirmation link.","pie-register");
			elseif ( 'payment_cancel' == $_GET['action'] )
                $message = __("You have cancelled the registration.","pie-register");
			elseif ( 'payment_success' == $_GET['action'] )
                $success = __("Thank you for registering. Login credentials will be sent soon.","pie-register");		
			elseif ( 'activate' == $_GET['action'] )
			{
				...
				if(sizeof($unverified )==1)
				{
					...
					if($user_login == $_GET['pie_id'])
					{
						...
						$success = __("Your account is now active","pie-register");	
					}
					else
					{
						 $message = __("Invalid activation key.","pie-register");	
					}
				}		
		...
		if ( !empty($success) )
			$pr_data .= '<p class="msg_div piereg_message">' . apply_filters('piereg_messages', __($success,"pie-register")) . "</p>";
		if($this->pie_post_array['success'] != "")
			$pr_data .= '<p class="msg_div piereg_message">'.apply_filters('piereg_messages', __($this->pie_post_array['success'],"pie-register")).'</p>';
		elseif($this->pie_post_array['error'] != "")
			$pr_data .= '<p class="msg_div piereg_login_error">'.apply_filters('piereg_messages', __($this->pie_post_array['error'],"pie-register")).'</p>';
    ...
    $pr_data .= '<h2 class="pr_renew_headding">'.(__("Renew Account","pie-register")).'</h2>';
    $pr_data .= '<div style="text-align:right;"><span class="field_note">* '.(__("Required Field(s)","pie-register")).'</span></div>';
	$pr_data .= '</div>';
      	...
      	<ul id="pie_register">';
			if(!empty($user_array) && $is_renew)
			{
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label>'.(__("User Name","pie-register")).'</label>
						<span>'.($user_array['username']).'</span>
					</div>
				</li>';
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label>'.(__("E-mail","pie-register")).'</label>
				...
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label for="x_card_num">'.(__("User Name","pie-register")).'</label>
						<input type="text" class="input_fields piereg_validate[required]" id="user_name" name="user_name" autocomplete="off" value="'.((isset($_POST['user_name']))?esc_attr($_POST['user_name']):"").'" aria-required="true" aria-invalid="false">
						<span class="required">*</span>
					</div>
				</li>';
				$pr_data .= '<li class="fields">
					<div class="fieldset">
						<label for="x_card_num">'.(__("Password","pie-register")).'</label>
			...
			$pr_data .= '<li class="fields">
				<div class="fieldset">
					<label for="x_card_num">'.(__("Select Payment","pie-register")).'</label>';
					...
					if(intval($piereg_form_pricing_fields['form_id_'.$user_registered_form_id]['field_as']) == 0){
						...
						$pr_data .= '<option value="">'.__("Select","pie-register").'</option>';
						$pr_data .= $allowed_payment_gateways;
						//$pr_data .= $send_data."</select>"; # Works need to be done when recurring payment enable.
						$pr_data .= "</select>";
					}
			...
			if( function_exists( 'wp_nonce_field' )) 
				$pr_data .=  wp_nonce_field( 'piereg_wp_renew_account_nonce','piereg_renew_account_nonce'); 
			
            $pr_data .= '
			</li>
			<li class="fields">
				<p class="submit">
    	          <input type="submit" value="'.(__("Renew Account","pie-register")).'" class="button button-primary button-large" id="pie_renew" name="pie_renew">
	            </p>
			</li>
      	</ul>   
      </form>
